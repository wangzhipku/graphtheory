...
//  Compare_++
//
//  Created by wangzhi on 2018/8/3.
...
int main(int argc, const char * argv[]) {
	//string data_path = "H:\\my_code\\ionc_ridus_series\\ionc_ridus_series\\data\\config\\";
	//string data_path = "C:\\Users\\王志\\Desktop\\0103去重后data\\2d\\";
	string data_path ;
	...
	double max2 = 0;
	double temp;
	char name1[100] = "1570-0_2d.config", name2[100] = "16220-0_2d.config", name4[5];
	for (i = 0; i < argc; i++)
	{
		if (i == 1)
		{
	...
	//{
	//	cout << atom_name[cell_a.type[i]] << " " << cell_a.if_lonely[i] << " " << cell_a.my_classify[i] << endl; 
	//}
	//for (i = 0; i < cell_b.num; i++)
	//{
	//	cout << atom_name[cell_b.type[i]] << " " << cell_b.if_lonely[i] << " " << cell_b.my_classify[i] << endl;
	//}
	...
	//for (i = 0; i < cell_a.num; i++)
	//{
	//	cout << a[i].if_positive << "," << a[i].my_clsssif << "," << a[i].if_lonely << endl;
	//}
	//for (i = 0; i < cell_b.num; i++)
	//{
	//	cout << b[i].if_positive << "," << b[i].my_clsssif << "," << b[i].if_lonely << endl;
	//}
				...
				for (ii = 0; ii < in.num; ii++)
				{
					...
					if (flag == 0)
					{
						...
						else if (if_lonely == 3)//会尝试连接金属或者非金属
						{
							...
							else if (in.my_classify[ii] == 2)//连接非金属
							{
...
void cell::judge_lonely(vector<vector<double>>max_ions)
{
	...
	{
		if (my_classify[i] == 2 || my_classify[i] == 3)
		{
			if_lonely[i] = 3;
		}
		else
